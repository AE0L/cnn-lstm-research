...
def selectDate(choice, userid):
    ...
    if choice=="Okay":
        #Create a Label
        Label(user, text= "Start Date", background= 'gray61', foreground="white", font=("ariel 12"), justify=LEFT).pack(padx=20,pady=20)
        #Create a Calendar using DateEntry
        sinceDate = DateEntry(user, width= 16, background= "lightblue", foreground= "white",bd=2, date_pattern='mm/dd/yyyy')
...
def userDetails(screen_name, userid, accAuth):
    ...
    screenname_label = ttk.Label(user, text="Screen Name: " + str(screen_name), font=('Helvetica', 12), justify=LEFT)
    screenname_label.pack(padx=20, pady=15)

    userid_label = ttk.Label(user, text="User ID: " + str(userid), font=('Helvetica', 12), justify=LEFT)
    userid_label.pack(padx=20, pady=15)

    accAuth_label = ttk.Label(user, text="User Auth: " + str(accAuth), font=('Helvetica', 12), justify=LEFT)
    accAuth_label.pack(padx=20, pady=15)
    
    back = ttk.Button(user, text="Back", style='success.TButton', width=8, command=lambda: selectDate("Back",userid))
    back.pack(padx=5, pady=10, side=RIGHT)

    okay = ttk.Button(user, text="Okay", style='success.TButton', width=8, command=lambda: selectDate("Okay",userid))
...
username_button = ttk.Button(user, text='Find User', style='success.TButton', command = get_value)
